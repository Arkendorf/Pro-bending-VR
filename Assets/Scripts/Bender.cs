...
public class Bender : MonoBehaviour
{
    ...
    [Tooltip("The position of the player's head")]
    public Transform headPos;
    [Tooltip("The position of the hand controlling the bending")]
    ...
    void Start()
    {
        // If no hand transform is given, use the transform of the gameobject this script is attached to
        if (!handPos)
        {
            handPos = transform;
        }
        // Set the old position to the current position for the first frame (to prevent the hand from firing immediately)
    ...
    void Update()
    {
        ...
        shoulderPos = headPos.position + headPos.rotation * shoulderOffset;
    ...
    private void Shoot()
    {
        ...
        GameObject elementObj = Instantiate(element, handPos.position, Quaternion.identity);
